...
public class WordChecker : MonoBehaviour
{
    ...
    private void SquareSelected(string letter, Vector3 squarePosition, int sqaureIndex)
    {
        if (_assignedPoints == 0)
        {
            _rayStartPosition = squarePosition;
            _correctSquareList.Add(sqaureIndex);
            _word += letter;
    ...
    {
        foreach(var seacrrhingWord in currentGameData.selectedBoardData.SearchWords)
        {
            if (_word == seacrrhingWord.Word)
            {
